...
class _SettingsScreenState extends State<SettingsScreen>
  ...
  /// 当前厂商的可用模型列表（builder 方法共用）。
  List<String> get _models => vendorModels[_vendor] ?? const <String>[];

  /// 厂商展示名映射（builder 方法共用）。
  Map<String, String> get _labels => vendorLabels;
  ...
  ListView _buildConnectionTab() {
    return ListView(
      ...
      children: [
        ...
        if ((_fetchedModels ?? _models).isNotEmpty) ...[
          const SizedBox(height: 4),
          Wrap(
            spacing: 4,
            children: [
              for (final m in (_fetchedModels ?? _models))
                ChoiceChip(
                  label: Text(m),
                  selected: _model == m,
  ...
  /// 设置页「智能」：模型/部门/记忆 + 各流程思考强度。
  ListView _buildIntelligenceTab() {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        // ── 模型与智能 ──
  ...
  /// 设置页「外观」：主题 / 更新 / GitHub / 版本。
  ListView _buildAppearanceTab() {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        // ── 外观与系统 ──
