...
> 状态：**P0-P4 已实施（2026-08-16，CI 全绿，代码与本文档对齐）**。实施记录见
> `docs/HANDOFF.md` 记忆子系统节，真机验证清单见 `docs/VERIFICATION.md`。
> 调研（2026-08-16）全部闭环，报告见 `docs/research/`（记忆对标 / Claude 插件 / 搜索引擎 / 分词词库）。
...
> 实施状态（2026-08-16）：**P0-P4 全部落地，CI 全绿**（详见 HANDOFF）。剩余：
> P2 决策点注入（trivial 门控已部分覆盖省 token）、异步委派取消/通知 UI、
> 真机验证（VERIFICATION.md 清单）。

- **P0（检索端）** ✅：§3 核心表 + dart_jieba + 词库资产 + FTS 热词检索 + `memory_search`/`memory_read` + `prefetchRecall` 真实现。⚠️ 真机 FTS5 验证待做（§2 风险）。
- **P1（图建构 + 置信度）** ✅：自动标签 + 知识层级边 + 证据层 + Beta 置信度 + 注入门控。
- **P2（异步 + 摘要）** ✅：prefetch 有界 8s + trivial 门控 + 摘要层（回合后快模型总结 + snippet 注入）。
- **P3（Agent 层，DSH 1-5）** ✅：Goal（证据驱动）+ 异步委派 + pruner + 技能目录注入 + 记忆对账。
- **P4（学习描绘）** ✅：学习事件=记忆观察 + 可提取性/遗忘节律 + 画像投影（证据驱动）+ 笔记库入库。
